  ...
  <div class="ag rise" style="--ac:${e.color};animation-delay:${t}s"><div class="agtop"><span class="agbot"><span class="head"></span><span class="eyes"><i></i><i></i></span></span><span class="agstat ${e.status}"><span class="d"></span>${e.status===`live`?`Live`:e.status===`beta`?`Beta`:`Soon`}</span></div><div class="role">${e.role}</div><h4>${e.name}</h4><p>${e.desc}</p></div>`;function me(e){let t=e.caps?{id:`${e.id}-2b`,theme:`light`,title:`Sector capabilities`,html:`
    ...
    <div class="slidebody">
      ...
      <div class="capgrid rise" style="animation-delay:.2s;grid-template-columns:repeat(5,1fr);margin-top:16px;">
        <div class="cap"><span class="ci"><svg viewBox="0 0 24 24">${w.layers}</svg></span><h4>Master data drift</h4><p>The same job carries different names, grades and rates across plants and contractors. You cannot count what you cannot name consistently.</p></div>
        <div class="cap"><span class="ci"><svg viewBox="0 0 24 24">${w.clock}</svg></span><h4>Demand set by habit</h4><p>Tomorrow's requirement is yesterday's deployment. Production plan, maintenance calendar and absenteeism never enter the number.</p></div>
        <div class="cap"><span class="ci"><svg viewBox="0 0 24 24">${w.users}</svg></span><h4>Contractor-built rosters</h4><p>Vendors are paid on deployed heads, so rosters carry a buffer. Nobody inside the plant sees planned versus actual per shift.</p></div>
        <div class="cap"><span class="ci"><svg viewBox="0 0 24 24">${w.chart}</svg></span><h4>No benchmark</h4><p>Workers per tonne, per line or per shift is not compared across plants or contractors, so the best-run unit never sets the standard.</p></div>
        <div class="cap"><span class="ci"><svg viewBox="0 0 24 24">${w.plug}</svg></span><h4>Disconnected systems</h4><p>SAP holds the PO, the gate holds entries, attendance holds punches, payroll holds hours. Reconciling them is a person's job — 155 people, in fact.</p></div>
      </div>
    ...
    <div class="slidebody split">
      <div>
        ...
        <div class="featlist">
          ${b(w.layers,`One position and skill master`,`Legacy job titles across plants and contractors mapped to a single position code with skill grade and wage category — the way Reliance runs position codes and budgets across 3,000+ vendors.`,.22)}
          ${b(w.doc,`Vendor, licence and work order as parents`,`Every worker hangs off a work order; every work order off a vendor with a licence and EC policy on file. At BPCL, PO and contract fields come straight from SAP and cannot be edited on our side.`,.3)}
          ${b(w.map,`Site, zone and wage master`,`Zone-wise minimum wage by skill category, built as a central master — the same artefact we built with BPCL when their state wage data lived in files.`,.38)}
          ${b(w.check,`Data-completeness enforced`,`A record missing what payroll or compliance needs says so on the profile and blocks the step. No silent gaps.`,.46)}
        </div>
      </div>
      <div class="rise" style="animation-delay:.35s">
    ...
    <div class="slidebody split">
      <div>
        ...
        <div class="featlist">
          ${b(w.clock,`Roster generated from the forecast`,`Required headcount per shift and skill becomes a published roster — shift cards, weekly grid, bulk upload — with vendors filling names against it, not above it.`,.22)}
          ${b(w.shield,`Hard caps, not guidelines`,`Work-order sanctioned strength and CLRA licensed strength cap every roster. The 51st worker against a 50-worker licence is refused, not reconciled later.`,.3)}
          ${b(w.lock,`The gate enforces the roster`,`A gate pass is valid only for a rostered worker with induction, verification and EC cover in place. Unplanned deployment does not enter. This is the BPCL gate-pass and auto-termination logic, applied to a plan.`,.38)}
          ${b(w.eye,`Planned vs actual, live`,`Plant leadership sees required, rostered and inside-the-gate per area, per shift, as it happens — the number the 155 people used to assemble by hand.`,.46)}
        </div>
      </div>
      <div class="rise" style="animation-delay:.35s">
    ...
    <div class="slidebody split">
      <div>
        ...
        <div class="featlist">
          ${b(w.book,`Wage Board as configuration, not judgement`,`Each Wage-Board category, its rate, OT multiple, holiday and shift rules sit in the rules engine. Every computation is the same, every time, and carries its own audit trail — which is also what a union asks to see.`,.22)}
          ${b(w.users,`A redeployment plan, not a headcount plan`,`Of 155+, roughly a third remain as exception handlers and plant time-office leads. The rest move to work the plant is short of: vendor governance, safety observation, quality checks, contractor audits — roles the new system creates demand for.`,.3)}
          ${b(w.clock,`Phased by plant, measured by control group`,`One plant first, with a comparison plant running as-is. Numbers are published to the works committee before the next plant moves.`,.38)}
          ${b(w.shield,`PwC owns the IR and change track`,`Union engagement, communication and the redeployment programme sit with PwC. BetterPlace supplies the system, the data and the evidence.`,.46)}
        </div>
      </div>
      <div class="rise" style="animation-delay:.35s">
    ...
    <div class="slidebody split">
      <div>
        ...
        <div class="featlist">
          ${b(w.doc,`Vendors, contracts and work orders from SAP`,`PO and contract data sourced from SAP and locked; labour licence and EC policy tracked per contract, with licence records auto-created once a contract crosses the 20-worker threshold.`,.24)}
          ${b(w.lock,`Gate pass, renewal and auto-termination`,`Passes issued and renewed on rules; workers auto-terminated on gate-pass expiry, attendance lapse or stage movement — with persona-specific rules down to transporter drivers and helpers.`,.32)}
          ${b(w.shield,`Compliance deviation workflows and wage master`,`Deviation approval and resubmission flows, downloadable compliance reports, and a central minimum-wage master by zone and skill category feeding payroll, ESI and full-and-final.`,.4)}
        </div>
      </div>
      <div class="rise" style="animation-delay:.35s">
    ...
    <div class="slidebody split">
      <div>
        ...
        <div class="featlist">
          ${b(w.layers,`Position and budget control`,`Position codes and budget detail as governing masters — position dashboards and budget reports at site-admin level, so deployment is planned against sanctioned positions, not requests.`,.24)}
          ${b(w.users,`Vendor and sub-vendor hierarchy at scale`,`Sub-contractor capture, bulk onboarding cycles at tens of thousands of records, and Kafka-based archival built specifically for attendance volume.`,.32)}
          ${b(w.plug,`Attendance into the enterprise warehouse`,`Attendance, leave and regularisation views delivered against a shared data dictionary for Jio and Retail’s central analytics dashboard.`,.4)}
        </div>
      </div>
      <div class="rise" style="animation-delay:.35s">
